...
export default function Screen5Improvements({ framework }: Props) {
  ...
  const Delta = ({ val, base, lower = false }: { val: number; base: number; lower?: boolean }) => {
    ...
    return (
      <span className={`ml-1 text-xs ${good ? "text-green-500" : "text-red-500"}`}>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {/* Framework reminder */}
      <div className="bg-[#1a2035] border border-slate-700 rounded-xl px-5 py-3 text-sm text-slate-300 flex flex-wrap gap-4">
        <span><strong className="text-slate-100">Outlook:</strong> {framework.outlook}</span>
        <span><strong className="text-slate-100">Risk:</strong> {framework.risk_tolerance}</span>
        <span><strong className="text-slate-100">Goal:</strong> {framework.goal}</span>
        <span><strong className="text-slate-100">Portfolio:</strong> {framework.portfolio_name}</span>
        <span><strong className="text-slate-100">Horizon:</strong> {framework.horizon}yr</span>
      </div>

      {/* Base summary */}
      {baseMetrics && (
        <div className="bg-[#1a2035] rounded-xl border border-slate-700 p-5">
          <h3 className="text-base font-bold text-slate-100 mb-3">Base Portfolio Metrics</h3>
      ...
      <div className="flex items-center gap-4">
        <button
          onClick={run}
          disabled={loading}
          className="bg-teal-600 hover:bg-teal-700 text-white font-semibold py-2.5 px-6 rounded-lg transition-colors disabled:opacity-50 flex items-center gap-2"
        >
        ...
        {improvements.length > 0 && (
          <div className="flex gap-2 ml-auto">
            <button onClick={exportCsv} className="bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-600 font-semibold py-2 px-4 rounded-lg text-sm transition-colors">
              ⬇ Export CSV
            </button>
            <button onClick={exportPdf} className="bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-600 font-semibold py-2 px-4 rounded-lg text-sm transition-colors">
              ⬇ Export PDF
            </button>
          </div>
        )}
      ...
      {improvements.length > 0 && (
        <div className="bg-[#1a2035] rounded-xl border border-slate-700 overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-700 bg-[#0f1629]">
            <span className="font-semibold text-slate-300">Top {improvements.length} Improvement Candidate{improvements.length > 1 ? "s" : ""}</span>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700">
                  <th className="px-4 py-3 text-left font-semibold bg-[#0f1629] text-slate-400 text-xs uppercase">Rank</th>
                  <th className="px-4 py-3 text-left font-semibold bg-[#0f1629] text-slate-400 text-xs uppercase">Note ID</th>
                  <th className="px-4 py-3 text-left font-semibold bg-[#0f1629] text-slate-400 text-xs uppercase">Type</th>
                  <th className="px-4 py-3 text-right font-semibold bg-[#0f1629] text-slate-400 text-xs uppercase">Alloc %</th>
                  <th className="px-4 py-3 text-right font-semibold bg-[#0f1629] text-slate-400 text-xs uppercase">New Sharpe</th>
                  <th className="px-4 py-3 text-right font-semibold bg-[#0f1629] text-slate-400 text-xs uppercase">New %Neg</th>
                  <th className="px-4 py-3 text-right font-semibold bg-[#0f1629] text-slate-400 text-xs uppercase">New Shorty</th>
                  <th className="px-4 py-3 text-right font-semibold bg-[#0f1629] text-slate-400 text-xs uppercase">Income Boost</th>
                  <th className="px-4 py-3 text-right font-semibold bg-[#0f1629] text-slate-400 text-xs uppercase">Score</th>
                  <th className="px-4 py-3 text-center font-semibold bg-[#0f1629] text-slate-400 text-xs uppercase">Chart</th>
                </tr>
              </thead>
              <tbody>
                {improvements.map((imp, idx) => (
                  <React.Fragment key={imp.note_id}>
                    <tr className={`border-b border-slate-700/50 hover:bg-[#232b45] transition-colors ${idx === expandedIdx ? "bg-teal-900/20" : ""}`}>
                      <td className="px-4 py-3 font-bold text-teal-400">#{idx + 1}</td>
                      <td className="px-4 py-3 font-mono font-semibold text-slate-200">{imp.note_id}</td>
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-slate-300">{imp.alloc_pct}%</td>
                      ...
                      <td className="px-4 py-3 text-right">
                        {imp.income_boost > 0
                          ? <span className="text-green-500 font-semibold">+{(imp.income_boost * 100).toFixed(4)}%</span>
                          : <span className="text-slate-500">—</span>}
                      </td>
                      <td className="px-4 py-3 text-right font-mono text-xs text-slate-400">{imp.score.toFixed(4)}</td>
                      <td className="px-4 py-3 text-center">
                        <button
                          onClick={() => loadHistogram(idx)}
                          disabled={loadingHist[idx]}
                          className="text-xs bg-teal-900/50 hover:bg-teal-800/50 text-teal-400 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                        >
                    ...
                    {expandedIdx === idx && histograms[idx] && (
                      <tr>
                        <td colSpan={10} className="px-4 py-4 bg-[#0f1629]">
                          <Plot
